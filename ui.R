...
ui <- fluidPage(theme = shinytheme("simplex"),
  ...
  navbarPage(title = div("COVID-19 Dashboard", style = "padding-left: 10px"),
    ...
    collapsible = TRUE,
    fluid       = TRUE,
    tabPanel("Overview", icon = icon("user-friends"), page_overview, value = "page-overview"),
    ...
    navbarMenu("More", icon = icon("question-circle"),
               tabPanel(tags$a("", href = "https://github.com/theodoraputrina-gif/Covid-19-Dashboard", target = "_blank",list(icon("github"), "Resources"))),
               tabPanel(tags$a("", href = "https://www.linkedin.com/in/theodora-putrina-0927141a2", target = "_blank",list(icon("linkedin"), "My Profile"))),
    ...
    header.append('<div style=\"float:right\"><a target=\"_blank\" href=\"https://www.youtube.com/watch?v=cNcLxXbFEEA\"><img src=\"nana.jpeg\" alt=\"alt\" style=\"float:right;width:33px;padding-top:10px;margin-top:-50px;margin-right:10px\"> </a></div>');
    console.log(header)")
    )
  )
)